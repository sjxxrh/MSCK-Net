...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None, pos_embed=None, size=None) -> torch.Tensor:
        residual = src
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, src_mask=None, pos_embed=None, size=None) -> torch.Tensor:
        ...
        for layer in self.layers:
            output = layer(output, src_mask=src_mask, pos_embed=pos_embed, size=size)
    ...
    def forward(self, feats):
        ...
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):     
                ...
                memory = self.encoder[i](src_flatten, pos_embed=pos_embed, size=(h, w))
